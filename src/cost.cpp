...
{
  // ref to: http://www.dyn4j.org/2010/01/sat/
  Vector2d trans0; 
  trans0 << x0, y0;
  Vector2d trans1; 
  trans1 << x1, y1;
...
int Cost::check_collision_on_trajectory(TrajectoryXY const &trajectory, std::map<int, vector<Coord> > &predictions)
{
  ...
  while(it != predictions.end()) {
    ...
    for (int i = 0; i < PARAM_MAX_COLLISION_STEP; i++) { // 25
...
bool Cost::check_max_capabilities(TrajectoryXY const &traj)
{
  // check the max speed, acceleration and jerk
  double vx, ax, jx;
  double vy, ay, jy;
  double vel, acc, jerk;
  double max_vel = 0;
  ...
  assert(traj.x_vals.size() == traj.y_vals.size()); // dim_x and dim_y
...
{
  // compute the cost for the trajectory in given target
  ...
  if (check_max_capabilities(trajectory)) // true or false
  {
    cost_feasibility += 1;
  }
  // 
  cost_ = cost_ + PARAM_COST_FEASIBILITY * cost_feasibility; // 10000
  ...
  // 
  cost_ = cost_ + PARAM_COST_SAFETY * cost_safety; // 1000

  // [3] LEGALITY cost
  // cost_ = cost_ + PARAM_COST_LEGALITY * cost_legality; // 100

  // [4] COMFORT cost
  // cost_ = cost_ + PARAM_COST_COMFORT * cost_comfort; // 10
