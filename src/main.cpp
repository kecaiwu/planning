    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          // previous path data given by the planner
          ...
          // how many waypoint in previous path's
          int prev_size = previous_path_xy.x_vals.size();

          // create sd trajectory at the frist time
          if (start) {
            ...
            // define a struct to organize previous path
            ...
            // prediction other vehicles location in the future 1 second
            Predictions predictions = Predictions(sensor_fusion, car, PARAM_NB_POINTS);
            // generate self vehicle behavior
            Behavior behavior = Behavior(sensor_fusion, car, predictions);
            // target location, target velocity, target acceleration with several cases
            vector<Target> targets = behavior.get_targets(); // lane, velocity, time and accel
            // generate self vehicle trajectory
            Trajectory trajectory = Trajectory(targets, map, car, previous_path, predictions);
            // ----------------------------------prediction, behavior, trajectory----------------------------------------

            // choose the trajectory with min cost
            ...
            // cout<<"next_x_vals: "<<next_x_vals[0]<<" "<<"next_y_vals: "<<next_y_vals[0]<<endl;

            // for next trajectory generation as previous path
            ...
            if (target_lane != car.lane) {
              cout<<"CHANGE LANE WITH LOWEST COST"<<endl;
              // cout<<"car.lane: "<<car.lane<<" "<<"target_lane: "<<target_lane<<endl;
              // cout<<"cost value: "<<min_cost<<endl;
            }
